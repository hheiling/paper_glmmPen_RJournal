...
sbatch --array=1-400 -N 1 -t 72:00:00 --mem=2g -n 1 --output=var_resp50_%a.out --wrap="R CMD BATCH select_50Cov_var_restrict.R var_resp50_$SLURM_ARRAY_TASK_ID.Rout"
...
* "extract_results_select_50Cov_var_restrict.R" - Takes coefficient estimate and timing results from the "select_50Cov_var_restrict.R" simulations and creates the "select_50Cov_var_restrict.RData" object
...
* "replication_select_50Cov_var_restrict.R" - Reads in "select_50Cov_var_restrict.RData" object (which includes relevant simulation output from the "select_50Cov_var_restrict.R" simulations) and creates a table of summary statistics: average fixed effects coefficients for the true predictors, true and false positive percentages for the fixed and random effects, and median time in hours to complete variable selection procedure.
...
source(sprintf("%s/replication_select_50Cov_var_restrict.R", path))
```
