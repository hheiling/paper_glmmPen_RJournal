# Compare model selection criteria options within glmmPen package 
## BIC-ICQ (default), BICh (hybrid BIC), BIC (N = number total observations)

library(stringr)

# Specify path to simulation results
path = "~/Longleaf/RJournal_GitHub/model_select_comparison"
...
sim_labels = str_c("Beta_1",
                   "_K_",rep(c(5,10),each=6),
                   "_SD_",rep(rep(c(1,round(sqrt(2),2)),each=3),times=2),
                   "_",rep(c("BIC-ICQ","BICh","BIC"),times=4))
...
for(s in 1:length(sims)){
  ...
  beta_mat = matrix(0, nrow = length(files), ncol = 11)
  vars_mat = matrix(0, nrow = length(files), ncol = 11)
  ...
  res[[sim_labels[s]]] = list(beta_mat = beta_mat, vars_mat = vars_mat, time_vec = time_vec)
}

save(res, file = sprintf("%s/model_select_comparison.RData",path_save))
